...
class EmailService {
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <h3>🔗 System Access</h3>
            <p>You can access the system at: <a href="${process.env.NEXTAUTH_URL || 'https://inegolintra.vercel.app/'}" style="color: #ff6b35;">${process.env.NEXTAUTH_URL || 'https://inegolintra.vercel.app/'}</a></p>
  ...
  async sendPermissionDecisionEmail(
    agentEmail: string, 
    agentName: string, 
    requestType: string,
    ...
    const subject = `Permission Request ${decision}: ${requestType}`
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { 
            background: linear-gradient(135deg, ${isApproved ? '#28a745, #34ce57' : '#dc3545, #e74c3c'}); 
            color: white; 
            padding: 30px 20px; 
            text-align: center; 
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>${isApproved ? '✅' : '❌'} Permission Request ${decision}</h1>
            <p>Your request has been ${isApproved ? 'approved' : 'rejected'}</p>
          </div>
          
          <div class="content">
            <h2>Hello ${agentName},</h2>
            <p>Your permission request has been processed by the administration team.</p>
            
            <div class="request-details">
              <h3>📋 Request Details</h3>
              <p><strong>Request Type:</strong> ${requestType}</p>
              <p><strong>Period:</strong> ${new Date(startDate).toLocaleDateString('en-US')} - ${new Date(endDate).toLocaleDateString('en-US')}</p>
              <p><strong>Your Reason:</strong> ${reason}</p>
              <p><strong>Status:</strong> <span class="status-badge">${decision}</span></p>
              <p><strong>Decision Date:</strong> ${new Date().toLocaleDateString('en-US')}</p>
            </div>
            
            ${adminNote ? `
            <div class="admin-note">
              <h4>💬 Administrator's Note:</h4>
              <p>${adminNote}</p>
            </div>
            ` : ''}
            ...
            ${isApproved ? `
            <ul>
              <li>Your permission request has been approved</li>
              <li>Please coordinate with your supervisor for coverage during your absence</li>
              <li>Ensure all pending tasks are completed or delegated before your leave</li>
              <li>Contact HR if you have any questions about your approved leave</li>
            </ul>
            ` : `
            <ul>
              <li>Your permission request has been rejected</li>
              <li>Please review the administrator's note above if provided</li>
              <li>You may submit a new request with additional information if appropriate</li>
              <li>Contact HR if you need clarification about the decision</li>
            </ul>
            `}
            
            <div class="footer">
              <p>This email was automatically generated by the Inegol Intranet HR system.</p>
              <p>If you have any questions, contact the HR administrator.</p>
              <hr style="margin: 20px 0; border: none; border-top: 1px solid #ddd;">
              <p style="font-size: 12px; color: #999;">
                © ${new Date().getFullYear()} Inegol Law Firm. All rights reserved.
              </p>
